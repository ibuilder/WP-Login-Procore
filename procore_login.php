...
/**
 * @package           Procore API Login
 *
 * @wordpress-plugin
 * Plugin Name:       Procore API Template Log In
 * Plugin URI:        
...
if (! defined('PROCORE_API_PATH')) {
	define('PROCORE_API_PATH', plugin_dir_path(__FILE__));
}

if (! defined('PROCORE_API_URL')) {
	define('PROCORE_API_URL', plugin_dir_url(__FILE__));
}

$class_name = 'Procore_API_Login';
if ( ! class_exists($class_name) ) :

class Procore_API_Login
{
	/**
	 * The unique prefix for Procore API.
	 *
	 * @since    1.1.0
	 * @access   protected
	 * @var      string    $prefix         The string used to uniquely prefix for Procore API.
	 */
	protected $prefix;
	...
	public function run()
	{
		...
		require_once(PROCORE_API_PATH .'/admin/register_settings.php');
		require_once(PROCORE_API_PATH .'/admin/settings.php');
		require_once(PROCORE_API_PATH .'/admin/admin-page.php');

		require_once(PROCORE_API_PATH .'/settings.php');
		require_once(PROCORE_API_PATH .'/class_api.php');

		$this->procore = new Procore_API();
		$this->settings();
		...
		add_action ('login_social_urls', array($this, 'procore_urls'));
	}
	...
	public function procore_urls() {
		echo do_shortcode('[procore-form]');
	}

	public function enque_scripts() {
		wp_enqueue_style( 'procore_oauth', PROCORE_API_URL . 'assets/css/style.css', array(), '1.0' );
		wp_enqueue_style( 'bootstrap_grid', 'https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/4.3.1/css/bootstrap-grid.css', array(), '1.0' ); 
	}
	...
	{		
		if (is_page($this->settings['pages']['login']['page_name'])) {
			$code = isset($_GET['code']) ? $_GET['code'] : '';
			if ($code != '') {
				try{
					$accessToken = $this->procore->get_access_token($code);
					$user_info = $this->procore->get_user_info();
...
<?php 
			...
			// Create the form
			echo '<div class="loginform-container"><div class="container">';
			?>
			<div class="row">
				...
				<div class="col-md-6 col-lg-4">
					...
					<div class="social-urls">
						<?php do_action('login_social_urls'); ?>						
					</div>					
				</div>
			</div>			
